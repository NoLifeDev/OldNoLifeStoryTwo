...
void obj::draw() {
	...
	case 1:
		if(movep) {
			ax = movew*sin(tdelta*2*PI/movep);
		} else {
			ax = movew*sin(tdelta/1000);
		}
		break;
	case 2:
		if(movep) {
			ay = moveh*sin(tdelta*2*PI/movep);
		} else {
			ay = moveh*sin(tdelta/1000);
		}
		break;
	case 3:
		ang = tdelta/mover*radtodeg;
		break;
	};
	delay += delta;
...
void back::draw() {
	...
	case 3:
		...
		break;
	case 4:
		x += double(rx*delta)/1000*5;
		if(cx==0) {return;}
		DRAWHOR
		break;
	case 5:
		y += double(ry*delta)/1000*5;
		if(cy==0) {return;}
		DRAWVERT
		break;
	case 6:
