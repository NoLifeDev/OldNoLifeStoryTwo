...
void Map::Load() {
	...
	currentmap = nextmap;
	node = mapNode;
	loadtime = nlstime.getElapsedTimeInMilliSec();
	...
	//if (theplayer != nullptr) delete theplayer; // to be save?
	...
	view.y = theplayer->y-350;
	view.vx = theplayer->x-400;
	view.vy = theplayer->y-350;
	view.tx = theplayer->x-400;
	view.ty = theplayer->y-350;
