...
void character::draw() {
	...
	OpenGL::drawNameTag(stats::name, x, y, stats::nametag);
	OpenGL::drawGuildName("The awesome test guild!", x, y);
}
	...
	if (!onladder) {
		if(ground) {
			if(ground->walk) {
				...
				double nspd;
				if(force) {
					nspd = max(abs(s1)-(fs*delta*walkForce/1000),(double)0)*sign(s1)+force;
				} else {
					nspd = max(abs(s1)-(fs*delta*walkDrag/1000),(double)0)*sign(s1)+force;
				}
				...
				vel.limit(ground->dir);
				vel.add(gravityAcc*delta/1000,90);
			...
			drag /= 1000000;
			vel.xcomp(max(abs(vel.xcomp()-force)-(delta*drag),(double)0)*sign(vel.xcomp()-force)+force);
			vel.ycomp(min(fallSpeed,vel.ycomp()));
		}
		//Move the player
		x += vel.xcomp()*delta/1000;
		y += vel.ycomp()*delta/1000;
		...
		if(!ground && !onladder) {
			double dis = vel.speed()*delta/1000;
...
void this_player::getequips (queue<wz::node>& parts) {
	...
	if (onladder || !((int)wz::top["Character"]["0000"+tostring(stats::skin)][state][tostring(frame)]["face"])) {
		parts.push(wz::top["Character"]["Hair"][tostring(stats::hair,8)]["backDefault"]);
	}
	else {
		parts.push(wz::top["Character"]["Face"][tostring(stats::face,8)]["default"]);
		parts.push(wz::top["Character"]["Hair"][tostring(stats::hair,8)]["default"]);
	}
